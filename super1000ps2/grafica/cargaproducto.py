...
class Carga(tk.Toplevel):
    def __init__(self, root,tituloFrame, titulo,frame_root,editar=None):
        ...
        self.l1=tk.Label(self.frame, text="Supermark")
        ...
        self.descripcion=tk.Label(self.frame2, text="Descripcion")
    ...
    def guardar_command(self):
        if len(self.nombreEntry.get())!=0 and len(self.descripcionText.get("1.0", "end"))!=0 and len(self.precioEntry.get())!=0 and len(self.cantidadEntry.get())!=0 and len(self.categoriaEntry.get())!=0:
            ...
            p.insertar()
            messagebox.showinfo("BBDD", "Producto insertado con exito")
            self.frame_root.mostrarProd()
            self.root.destroy()
        else:
            messagebox.showerror("BBDD", "No se pudo agregar el Producto, \ndebe completar todos los campos")
    ...
    def actualizar_command(self):
        ...
        if self.datos != "":
            ...
            p.modificar()
            messagebox.showinfo("BBDD", "Producto actualizado con exito")
            self.frame_root.mostrarProd()
            self.root.destroy()
        else:
            messagebox.showinfo("BBDD", "No se actualizo el Producto")
